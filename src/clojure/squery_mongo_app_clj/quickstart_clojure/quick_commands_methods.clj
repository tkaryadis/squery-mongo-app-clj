(ns squery-mongo-app-clj.quickstart_clojure.quick-commands-methods
  (:refer-clojure :only [])
...
;  The Java quickstart https://github.com/mongodb-developer/java-quick-start in squery-mongo using the Commands API
;  before starting , load the data (they are in the squery,app folder,the directory named data-test)
;  cd data-test
...
;;All updates in squery use pipelines,for updates with update operators
(defn update-commands []
  ...
  (println (update- :sample_training.grades
                    (uq (upsert {:student_id  10002. :class_id 10.})
  ...
  ;;findOneAndUpdate (findAndModify squery command will be used)
...
(defn update-methods []
  (let [grades (.getCollection (.getDatabase (defaults :client) "sample_training") "grades")]
    ...
    (println (.updateOne grades
                         ...
                         (o (UpdateOptions.)
                            {:upsert true})))
    ;;same not pipeline (squery doesnt wrap update operators,for updates it uses pipelines)
    (println (.updateOne grades
                         (d {:student_id  10002. :class_id 10.})
...
;;The above updates were pipeline updates,if we want to use update operators we can write MQL raw,squery doesnt wrap those
  ...
  (let [db (.withCodecRegistry (.getDatabase ^MongoClient (defaults :client) "sample_training") pojo-registry)
    ...
    (let [grade (.first (fq grades-coll (= :student_id 10003.)))]
      ...
      (let [newScores (ArrayList. (.getScores grade))
            ...
            ;;use of the driver method with squery arguments only
            ;;squery-mongo findAndModify cant take as argument a custom class
...
;;squery commands(corrensponds to MQL commands runCommand(...)) looks like code,and they are easy to use
(defn run-commands []
...
;;here we use the driver methods,with squery arguments
(defn run-methods []
  ;;here randomly we choose to get Documents that are Clojure maps
...
;;commands in squery are programmable and easy to use in squery (so 2 implementations are made for 2 ways to use squery)
;(run-commands)
;(run-methods)
